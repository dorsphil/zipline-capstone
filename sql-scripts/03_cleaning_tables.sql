...
WHERE NEST_NAME IS NULL OR TRIM(NEST_NAME) = ''
   OR PRIORITY IS NULL OR TRIM(PRIORITY) = ''
   OR DELIVERY_KEY IS NULL OR TRIM(DELIVERY_KEY) = ''
   OR HEALTH_FACILITY_NAME IS NULL OR TRIM(HEALTH_FACILITY_NAME) = ''
   OR N_UNITS IS NULL
   OR TIME_ORDER_CONFIRMED_LOCAL IS NULL;
...
WHERE FACILITY_NAME IS NULL OR TRIM(FACILITY_NAME) = ''
   ...
   OR SITE_ID IS NULL OR SITE_ID = 0;
...
-- Fix District and Region in one table update
UPDATE facilities 
SET 
    DISTRICT = CASE WHEN (TRIM(DISTRICT) = '' OR DISTRICT IS NULL) THEN 'Unknown' ELSE DISTRICT END,
    REGION = CASE WHEN (TRIM(REGION) = '' OR REGION IS NULL) THEN 'Unknown Region' ELSE REGION END,
    SITE_ID = CASE WHEN SITE_ID = 0 THEN NULL ELSE SITE_ID END;
...
SELECT FACILITY_NAME, DISTRICT,
       REGION, SITE_ID
       FROM  facilities
       WHERE FACILITY_NAME = 'Asawinso SDA HSP' AND SITE_ID IS NULL; --since all blanks were set to NULL initially
...
SELECT *
	FROM deliveries
	WHERE TIME_DELIVERED_LOCAL < TIME_ORDER_CONFIRMED_LOCAL;
